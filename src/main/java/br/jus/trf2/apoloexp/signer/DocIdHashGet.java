...
public class DocIdHashGet implements IDocIdHashGet {
	...
	public void run(Request req, Response resp, AssijusSystemContext ctx) throws Exception {
		...
		try {
			...
			// Retrieve parameters
			sha1 = cstmt.getString(5);
			sha256 = cstmt.getString(6);
			pagecount = cstmt.getInt(7);
			dthrUltAtu = cstmt.getTimestamp(8);
		...
		if (sha256 == null && ApoloExpSignerServlet.getProp("pdfservice.url") != null) {
