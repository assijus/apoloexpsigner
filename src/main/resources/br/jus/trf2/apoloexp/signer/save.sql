...
begin
	...
	i_cpf := ?;
  	...
  	select count(*) into v_count from DocumentoArquivo da where da.CodSecao = i_codsecao and da.CodDoc = i_coddoc and da.Numtipmovarq is null;
  	if (v_count > 0) then
		o_status := 'Erro';
  		...
  		begin
	  		select DtHrUltAtu, TxtPdfCompr, NumPagesPdf into v_dthrultatu, v_pdfcomprimido, v_pdfnumpaginas from ExpedientePdf mtp where mtp.CodSecao = i_codsecao and mtp.CodDoc = i_coddoc and mtp.Status='convertidoempdf';
		...
		if (i_dthrultatu is not null and i_dthrultatu <> v_dthrultatu) then
			o_status := 'Erro';
  			...
  			-- obtem a descricao
		  	select 
				formata_exp(num) || ' - ' || (select c.Descr from TipoExpediente c where c.CodSecao = m.CodSecao and c.Codtipexp = m.codtipexp)
			into v_descr
			from     
				T_expediente m
			where     
			...
			-- grava nas tabelas
			documentoarquivo_i(i_codsecao,i_coddoc,v_seq,NULL,NULL,i_pdfcomprimido,NULL,NULL,v_descr,i_envelopecomprimido,v_dthrincl,NULL,i_pdfnumpaginas,'N',NULL,NULL,NULL,NULL,NULL,NULL);
			...
			update expedientepdf set status='assinado' where codsecao = i_codsecao and coddoc = i_coddoc and to_char(dthrultatu, 'dd/mm/yyyy hh24:mi') = to_char(v_dthrultatu, 'dd/mm/yyyy hh24:mi');
